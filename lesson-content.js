    ...
    '1-1': {
        ...
        story: `
            <div class="story-paragraph">
                <p>Blockchain is a distributed ledger technology that maintains a continuously growing list of records called blocks, linked and secured using cryptography, where each block contains a cryptographic hash of the previous block, a timestamp, and transaction data, creating an immutable chain resistant to data modification. First introduced through Bitcoin, blockchain enables peer-to-peer electronic systems to function without trusted central authorities, using network participants called nodes to validate transactions through consensus mechanisms like Proof of Work, where miners compete to solve cryptographic puzzles to add new blocks and receive rewards. The decentralized nature ensures no single point of failure exists, as blockchain data is distributed across thousands of computers worldwide, with each node maintaining a complete copy and independently verifying transactions according to consensus rules, allowing the network to continue operating even if many nodes fail. Immutability comes from the cryptographic design where modifying any transaction would require recalculating hashes for that block and all subsequent blocks, becoming exponentially difficult with each new block, while also requiring control of more than 50% of network computational power, making attacks economically unfeasible. Beyond digital currencies, blockchain technology enables smart contracts—self-executing programs that automatically enforce agreements—transforming blockchain into a global, decentralized computer capable of running applications without downtime, censorship, or third-party interference, enabling new services like DeFi and NFTs. Transparency and security are fundamental characteristics, as all transactions are publicly visible for independent verification, while cryptography ensures only private key holders can authorize transactions, and consensus mechanisms prevent malicious manipulation, creating a trustless model where participants rely on mathematical principles rather than trusting each other or intermediaries. The applications extend beyond finance to supply chain management, identity management, voting systems, and intellectual property protection, with emerging specialized blockchains optimized for different use cases, representing a paradigm shift toward open, transparent, and user-controlled digital systems that reshape how we interact, transact, and organize value in the digital age.</p>
            </div>
        `,
        resources: [
            {
